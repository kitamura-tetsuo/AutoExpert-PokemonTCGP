...
def play(state, game):
    ...
    for aid in legal_actions:
        ...
        elif "AttachEnergy" in aname or "Attach(" in aname:
            # Handle both formats: AttachEnergy(0, Fire) or Attach(Fire, 0)
            m = re.search(r"Attach(?:Energy)?\((?:(\d+), (.*?)|(.*?),\s*(\d+))\)", aname)
            if m:
                # If group 1 exists, it's AttachEnergy(pos, type). Otherwise it's Attach(type, pos)
                if m.group(1):
                    pos = int(m.group(1))
                    energy_type = m.group(2)
                else:
                    energy_type = m.group(3)
                    pos = int(m.group(4))

                action["type"] = "attach_energy"
                action["pos"] = pos
                action["energy_type"] = energy_type
